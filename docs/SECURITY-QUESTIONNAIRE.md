...
| 5 | What is the blast radius if the vendor is compromised? | The GitHub release tarball. There is no server holding your data, no session to hijack and no update channel that can push code — holt never self-updates. `holt audit` plus `gh attestation verify` detect a substituted package before it runs. |
...
| Data retention / deletion | Nothing is retained by the vendor. Delete the journal and the licence file and nothing of holt's remains. |
...
| Runtime dependencies | **Zero required.** Everything in the SBOM is optional; `npm install -g <github-release-tarball> --omit=optional` is a genuinely zero-dependency install, and CI proves the degraded path works and says so. |
...
| Signed releases | A GitHub artifact attestation over the release tarball, the SBOMs and the integrity manifest, plus a detached Ed25519 signature over `MANIFEST.sha256`. The public key is pinned in `src/supply-chain.mjs`; the private key is stored as the repository secret `HOLT_RELEASE_SIGNING_KEY`. `holt audit --require-signature` refuses if no valid signature is present. |
...
| Third-party binaries | Optional and never bundled: `ctags`, `enry`, `rg`, `jj`, `jscpd`. `holt setup` can download a pinned static `universal-ctags` (including a Windows build from `universal-ctags/ctags-win32`) and **refuses to install it on a SHA-256 mismatch**, or run your own package manager (§7.8); it can also install `enry` via `go install` when Go is available. `mount` is used on macOS/Unix to detect network filesystems for timeout escalation; `where` is the Windows equivalent of `command -v` for tool detection. Installing the backends yourself avoids all of it. |
...
| Question | Answer |
|---|---|
| Availability dependency | **None at runtime.** holt is a local binary; it keeps working if the vendor is offline, and it keeps working if the vendor is gone. |
...
8. **`holt doctor --install` can run your package manager, with `sudo` on Linux.** It prints the
   ...
   hook, MCP call or git action ever elevates. The indirect path is disclosed
   because a true sentence can still mislead. A capability that arrives through a
   child process is still a capability, and no in-process detector can see it.
   `holt audit --json` reports it under `statement.indirectNetwork`.
