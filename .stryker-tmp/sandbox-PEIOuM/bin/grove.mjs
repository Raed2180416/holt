...
ACTING  (these MUTATE; everything above is read-only)
  protect             git-lock every workstream holding unique work  [--dry-run]
  unprotect [<id>]    release grove's locks
...
/** Actions return structured results; print them plainly or as JSON. */
async function cmdAction(promise, opts) {
...
async function main() {
  ...
  if (cmd === 'protect') return cmdAction(protect(opts.cwd, opts), opts);
  if (cmd === 'unprotect') return cmdAction(unprotect(opts.cwd, { id: opts._[1] ?? null, ...opts }), opts);
  if (cmd === 'rescue') {
    if (!opts._[1]) { process.stderr.write(paint('red', 'grove rescue: needs a workstream id\n')); process.exit(2); }
    return cmdAction(rescue(opts.cwd, opts._[1], opts), opts);
  }
  if (cmd === 'rescued') return cmdAction(rescues(opts.cwd), opts);
  if (cmd === 'clean') return cmdAction(clean(opts.cwd, opts), opts);
