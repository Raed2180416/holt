...
export async function rescue(cwd, id, { dryRun = false, release = false, ...opts } = {}) {
  ...
  try {
    ...
    await gitOk(['add', '--all', '--force', '--', '.'], { cwd: ws.path, env, allowMutation: true });
    ...
    const check = await git(['diff', '--name-only', `${commit}`, '--', '.'], { cwd: ws.path });
    const captured = await git(['ls-tree', '-r', '--name-only', commit], { cwd: ws.path });
    const capturedFiles = captured.code === 0 ? captured.stdout.split('\n').filter(Boolean) : [];
    const missing = files.filter((f) => !capturedFiles.includes(f));

    if (missing.length) {
      return {
        ok: false, id, ref, commit,
        error: `rescue is INCOMPLETE — ${missing.length} file(s) not captured: ${missing.slice(0, 5).join(', ')}`,
        note: 'the worktree has NOT been released; nothing was deleted',
      };
    }
 ...
 */
export async function clean(cwd, { apply = false, branches = true, ...opts } = {}) {
  const { report } = await assess(cwd, opts);
