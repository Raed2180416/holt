...
export const MUTATIONS = [
  ...
  },
  {
    ...
    file: 'src/integrate/adapters.mjs',
    find: '      const rel = await relativeWithinAsync(repoRoot, file);\n      if (await holtOwnsFile(repoRoot, rel, receipt)) {\n        await fs.rm(file, { force: true });\n        results.push({ adapter: \'opencode\'',
    replace: '      const rel = await relativeWithinAsync(repoRoot, file);\n      if (true) { // mutated: filename/marker authorizes whole-file deletion\n        await fs.rm(file, { force: true });\n        results.push({ adapter: \'opencode\'',
    tests: ['test/e2e/integrate-upgrade.test.mjs'],
  },
  {
    ...
    file: 'src/integrate/adapters.mjs',
    find: '  if (!String(text).includes(PRE_COMMIT_MARKER)) return false;\n  const suffix =',
    replace: '  if (String(text).includes(PRE_COMMIT_MARKER)) return true; // mutated: marker means ownership\n  const suffix =',
    tests: ['test/e2e/integrate-upgrade.test.mjs'],
  },
  {
  ...
  },
  {
    ...
    file: 'src/git.mjs',
    find: '  const commandContext = await buildGitCommandContext(argv, cwd, env, timeout);',
  ...
  },
  {
    ...
    file: 'src/actions.mjs',
    find: "    const r = await gitOk(['hash-object', '-w', '--no-filters', '--', hashPath],",
    replace: "    const r = await gitOk(['hash-object', '-w', '--', hashPath], // mutated: conversion can rewrite rescue bytes",
    tests: ['test/e2e/git-execution-boundary.test.mjs'],
  },
  {
    id: 'clean-no-recheck',
  ...
  },
  {
    ...
    file: 'src/scan.mjs',
    find: '    mt = await git([\'merge-tree\', \'--write-tree\', baseOid, headOid], { cwd: root, timeout });',
