## holt 0.3.1 — the safety guarantees are now true on every path we could find

**0.3.0 made the claims. 0.3.1 is where they hold.** This release exists because the instruments
that were supposed to be checking holt were themselves broken, and once they were fixed the
product turned out to be failing open in several ordinary situations.

If you are running 0.3.0, upgrade. Several of these are the difference between holt protecting
your work and holt watching it go.
...
One command — no clone, no build. Built, installed and driven against a real repository on Linux,
macOS **and** Windows by CI before the file is attached. `holt.tgz` is a stable name, so this URL
keeps working; the versioned `holt-0.3.1.tgz` is attached alongside it.

### The guard was failing open. Five ways.

Each was reproduced before it was fixed, and each fix was re-checked red against the old code.
...
- **The hooks `holt integrate` installs disarmed the guard.** For Claude Code, integrate wires the
  ...
  brief hook first, which the installed configuration does on **every user message**, and the
...
- **The hook stalled every tool call** for as long as the host held stdin open — 27 seconds
  measured. Now 0.1 s.
...
- A source-stripping bug meant **43% of the codebase never reached the no-telemetry and
  path-comparison gates** — 95% of the CLI entrypoint among it. The no-telemetry guarantee survived
  the widened scan; it was under-verified, not false.
- The static-analysis ratchet **wrote a zero and passed** when the type checker could not run, and
  its config meant nothing was really being type-checked. 362 → 224 diagnostics with no product
  code changed.
- Both benchmark harnesses reported "correct" for runs that graded **nothing**. §1's headline
  "1000/1000 correct" had 300 verdicts ungraded and a summary line that divided the planted count
  by itself. Re-graded honestly, it is still 1000/1000 — the number was true and unverified, which
...
| | |
|---|---|
| the guard's cost on an ordinary command | **65 ms**, and flat in repository size |
| the guard's cost on a command that could destroy work | 993 ms — it pays for certainty only where work is at stake |
| holt's own memory, 31 worktrees on redis | **~11 MB** |
| verdict correctness at 1,000 worktrees | **1000/1000**, every denominator a real graded count |

Full conditions and reproduce commands: [BENCHMARKS.md](https://github.com/Raed2180416/holt/blob/main/BENCHMARKS.md).

### Honest limits

Host coverage is verified end to end on the hosts CI actually drives. The rest are wired from a
manifest that matches each host's published schema but have not each been driven live — `HOSTS.md`
labels which is which, and that labelling is generated from the manifest rather than written by
hand.
