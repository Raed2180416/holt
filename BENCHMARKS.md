...
| Instrument | Result |
|---|---|
| tests | 748 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **748 defined, 748 passing.**
A developer without opencode installed sees 747 passing and 1 skipped, and `npm test` says so.
