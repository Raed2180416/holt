...
| Instrument | Result |
|---|---|
| tests | 764 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **764 defined, 764 passing.**
A developer without opencode installed sees 763 passing and 1 skipped, and `npm test` says so.
