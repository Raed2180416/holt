...
| Instrument | Result |
|---|---|
| tests | 760 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **760 defined, 760 passing.**
A developer without opencode installed sees 759 passing and 1 skipped, and `npm test` says so.
