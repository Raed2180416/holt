...
| Instrument | Result |
|---|---|
| tests | 738 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **738 defined, 738 passing.**
A developer without opencode installed sees 737 passing and 1 skipped, and `npm test` says so.
