...
| Instrument | Result |
|---|---|
| tests | 765 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **765 defined, 765 passing.**
A developer without opencode installed sees 764 passing and 1 skipped, and `npm test` says so.
