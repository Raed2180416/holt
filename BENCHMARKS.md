...
| Instrument | Result |
|---|---|
| tests | 780 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
| deliberate-defect mutations | 46/46 killed (`npm run test:mutation`) — first run was 10/12; both survivors were real holes, fixed |
...
CI now installs opencode, so nothing skips and there is one number: **780 defined, 780 passing.**
