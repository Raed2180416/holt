...
| Instrument | Result |
|---|---|
| tests | 763 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **763 defined, 763 passing.**
A developer without opencode installed sees 762 passing and 1 skipped, and `npm test` says so.
