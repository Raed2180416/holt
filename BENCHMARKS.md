...
| Instrument | Result |
|---|---|
| tests | 750 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **750 defined, 750 passing.**
A developer without opencode installed sees 749 passing and 1 skipped, and `npm test` says so.
