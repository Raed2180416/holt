...
| Instrument | Result |
|---|---|
| tests | 751 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **751 defined, 751 passing.**
A developer without opencode installed sees 750 passing and 1 skipped, and `npm test` says so.
