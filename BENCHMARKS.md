...
| Instrument | Result |
|---|---|
| tests | 762 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **762 defined, 762 passing.**
A developer without opencode installed sees 761 passing and 1 skipped, and `npm test` says so.
