...
| Instrument | Result |
|---|---|
| tests | 758 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
...
CI now installs opencode, so nothing skips and there is one number: **758 defined, 758 passing.**
A developer without opencode installed sees 757 passing and 1 skipped, and `npm test` says so.
