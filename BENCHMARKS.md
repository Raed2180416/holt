...
| Instrument | Result |
|---|---|
| tests | 777 passing (`npm test`) — the count that EXECUTES on a clean CI runner |
| deliberate-defect mutations | 45/45 killed (`npm run test:mutation`) — first run was 10/12; both survivors were real holes, fixed |
...
CI now installs opencode, so nothing skips and there is one number: **777 defined, 777 passing.**
